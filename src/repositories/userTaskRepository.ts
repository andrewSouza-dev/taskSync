...
export interface UserTaskRepository {
    ...
    createTaskByUser(userid: number, taskId: number, role?: string): Promise<UserTask>

    // Atualizar task de um usuário (role ou campos do relacionamento)
    updateUserTask(userId: number, taskId: number, role?: string): Promise<UserTask>

    // Deletar task de um usuário (sem deletar a task global)
    deleteUserTask(userId: number, taskId: number): Promise<UserTask>
}   